...
export function resolveAnthropicMessagesUrl(baseUrl: string, provider: ProviderType): string {
  if (provider === 'anthropic-compatible') {
    // 如果已经是完整的 /messages 路径，直接使用；否则追加 /v1/messages
    // 兼容 Django 代理等场景，其 baseUrl 不包含 /v1/messages 后缀
    if (hasPathSuffix(baseUrl, '/messages') || hasPathSuffix(baseUrl, '/v1/messages')) {
      return trimTrailingUrlPathSlash(baseUrl)
    }
    return `${trimTrailingUrlPathSlash(baseUrl)}/v1/messages`
  }
