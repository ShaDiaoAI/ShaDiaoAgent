...
function main(): void {
  ...
  // 打印配置信息
  console.log(`\n${color.bgBlue}${color.bold} ShaDiaoAgent 打包工具 ${color.reset}\n`)
  ...
  // ── 步骤 4: 编译 shadiao-agent CLI 二进制 ──
  step++
  printStepStart(step, totalSteps, '编译 shadiao-agent CLI (bun --compile)')
  results.push(
    runStep('编译 shadiao-agent CLI', 'bun', ['run', 'build:cli'], { verbose: opts.verbose })
  )
  printStepResult(results[results.length - 1])
  if (!results[results.length - 1].success) return printSummary(results)
