#!/usr/bin/env bun
/**
 * 编译 shadiao-agent CLI 为自包含二进制，打进桌面构建。
 *
 * 设计：
 * - 用 `bun build --compile` 把 apps/cli 连同其 workspace 依赖
 *   （@shadiao/session-core / @shadiao/shared）打成单个自包含可执行档，
 *   用户机器无需安装 bun/node 即可运行。
 * - 输出到 apps/electron/resources/bin/，由 electron-builder 经 extraResources
 *   打进 process.resourcesPath/bin/，运行时由主进程注入 SHADIAO_CLI 暴露给 skill。
...
const binName = isWindows ? 'shadiao-agent.exe' : 'shadiao-agent'
...
console.log(`${color.cyan}[build:cli]${color.reset} 编译 shadiao-agent CLI → ${color.dim}${outFile}${color.reset}`)
