...
export interface ElectronAPI {
  ...
  /** 获取未暂存的变更文件列表 */
  getUnstagedChanges: (dirPath: string, sessionPath?: string, workspaceFilesPath?: string, extraPaths?: string[], sessionId?: string) => Promise<import('@shadiao/shared').UnstagedChangesResult>
  /** 获取单个文件的 diff */
  getFileDiff: (input: import('@shadiao/shared').GetFileDiffInput) => Promise<string>
  /** 获取未追踪文件内容 */
  getUntrackedContent: (input: import('@shadiao/shared').GetFileDiffInput) => Promise<string>
  /** 还原文件变更 */
  revertFile: (input: import('@shadiao/shared').RevertFileInput) => Promise<void>
  /** 获取文件新旧版本内容 */
  getDiffContents: (input: import('@shadiao/shared').GetFileDiffInput) => Promise<{ oldContent: string; newContent: string } | null>
  /** 列出 Git Worktree */
  listWorktrees: (repoPath: string, sessionId: string) => Promise<import('@shadiao/shared').WorktreeInfo[]>
  /** 获取 Worktree 相对于基准分支的全量变更 */
  getWorktreeChanges: (worktreePath: string, baseBranch: string, sessionId: string) => Promise<import('@shadiao/shared').UnstagedChangesResult>
  /** 在独立窗口打开当前文件预览 */
  ...
  /** 测试 MCP 服务器连接 */
  testMcpServer: (name: string, entry: import('@shadiao/shared').McpServerEntry) => Promise<{ success: boolean; message: string }>
  ...
  listSkillFiles: (workspaceSlug: string, skillSlug: string) => Promise<import('@shadiao/shared').SkillFileNode[]>

  /** 读取 Skill 目录下的子文件内容 */
  readSkillFile: (workspaceSlug: string, skillSlug: string, relativePath: string) => Promise<import('@shadiao/shared').SkillFileContent>
  ...
  /** 读取工作区 CLAUDE.md */
  readWorkspaceClaudeMd: (workspaceSlug: string) => Promise<import('@shadiao/shared').SkillFileContent>

  /** 写入工作区 CLAUDE.md */
  writeWorkspaceClaudeMd: (workspaceSlug: string, content: string) => Promise<void>

  /** 列出工作区 auto memory 文件树 */
  listWorkspaceAutoMemoryFiles: (workspaceSlug: string) => Promise<import('@shadiao/shared').SkillFileNode[]>

  /** 读取工作区 auto memory 文件 */
  readWorkspaceAutoMemoryFile: (workspaceSlug: string, relativePath: string) => Promise<import('@shadiao/shared').SkillFileContent>
  ...
  getWorktreeRepos: (workspaceSlug: string) => Promise<import('@shadiao/shared').WorkspaceWorktreeRepo[]>
  /** 添加 worktree 仓库到工作区配置 */
  addWorktreeRepo: (workspaceSlug: string, repo: import('@shadiao/shared').WorkspaceWorktreeRepo) => Promise<import('@shadiao/shared').WorkspaceWorktreeRepo[]>
  /** 从工作区配置移除 worktree 仓库 */
  removeWorktreeRepo: (workspaceSlug: string, repoPath: string) => Promise<import('@shadiao/shared').WorkspaceWorktreeRepo[]>
  ...
  systemOpenFile: (filePath: string, appName?: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<void>

  /** 扫描系统中可用的编辑器应用（仅 macOS） */
  scanEditors: () => Promise<import('@shadiao/shared').EditorApp[]>

  /** 查询本机为该文件类型注册的默认打开应用（含图标 dataURL） */
  getDefaultAppForFile: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<import('@shadiao/shared').DefaultAppInfo | null>
  ...
  /** 解析文件路径并读取内容（供内联预览使用） */
  resolveAndReadFile: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<{ resolvedPath: string; content: string } | null>

  /** 写入文本文件（供 Markdown 内联编辑使用） */
  writeTextFile: (filePath: string, content: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<boolean>

  /** 仅解析文件路径（供 PDF/图片等用 file:// 加载） */
  resolveFilePath: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<import('@shadiao/shared').ResolvedFileUrl | null>

  /** 为内联 PDF 预览生成临时 HTML 文件，返回文件路径 */
  preparePdfPreview: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<{ tmpHtmlUrl: string } | null>

  /** 读取文件为 base64（带路径校验，供内联图片预览等） */
  readBinaryBase64: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions, maxSize?: number) => Promise<string | null>

  /** DOCX 转 HTML（内联预览） */
  docxToHtml: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<{ resolvedPath: string; html: string } | null>

  /** XLSX/PPTX 转 HTML（内联预览） */
  officeToHtml: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<import('@shadiao/shared').OfficePreviewResult | null>
  ...
  /** 列出附加目录内容 */
  listAttachedDirectory: (dirPath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<FileEntry[]>

  /** 读取附加目录文件内容为 base64（限制在已附加目录范围内） */
  readAttachedFile: (filePath: string, sessionId?: string, workspaceSlug?: string) => Promise<string>

  /** 在文件管理器中显示附加目录文件 */
  showAttachedInFolder: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<void>

  /** 重命名附加目录文件/目录（无工作区路径限制） */
  renameAttachedFile: (filePath: string, newName: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<void>

  /** 移动附加目录文件/目录（无工作区路径限制） */
  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@shadiao/shared').FileAccessOptions) => Promise<void>
  ...
  /** 获取多 Bot 配置 */
  getFeishuMultiConfig: () => Promise<import('@shadiao/shared').FeishuMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveFeishuBotConfig: (input: import('@shadiao/shared').FeishuBotConfigInput) => Promise<import('@shadiao/shared').FeishuBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getFeishuMultiStatus: () => Promise<import('@shadiao/shared').FeishuMultiBridgeState>

  // --- 扫码注册 ---

  /** 启动扫码注册流程，等待用户扫码 + 飞书确认后返回 App ID/Secret */
  registerFeishuApp: () => Promise<import('@shadiao/shared').FeishuRegisterAppResult>
  /** 取消正在进行的扫码注册流程 */
  cancelFeishuRegistration: () => Promise<void>
  /** 监听二维码 URL 生成 */
  onFeishuRegisterQrcode: (callback: (payload: import('@shadiao/shared').FeishuRegisterAppQRCode) => void) => () => void
  /** 监听注册流程状态变化 */
  onFeishuRegisterStatus: (callback: (payload: import('@shadiao/shared').FeishuRegisterAppStatus) => void) => () => void
  ...
  /** 获取多 Bot 配置 */
  getDingTalkMultiConfig: () => Promise<import('@shadiao/shared').DingTalkMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveDingTalkBotConfig: (input: import('@shadiao/shared').DingTalkBotConfigInput) => Promise<import('@shadiao/shared').DingTalkBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getDingTalkMultiStatus: () => Promise<import('@shadiao/shared').DingTalkMultiBridgeState>
  ...
  /** 打开文件选择对话框（选择 .shadiao-backup 或 .shadiao-share） */
  ...
  getCharacter: (id: number) => Promise<{ success: boolean; data?: any; error?: string }>
  ...
  mySkins: () => Promise<{ success: boolean; data?: any[]; error?: string }>
  equipSkin: (characterId: number, skinId: number) => Promise<{ success: boolean; error?: string }>

  // ===== ShaDiaoAgent: 物品 =====
  getInventory: () => Promise<{ success: boolean; data?: any[]; error?: string }>
  equipItem: (inventoryId: number) => Promise<{ success: boolean; error?: string }>
  unequipItem: (inventoryId: number) => Promise<{ success: boolean; error?: string }>
...
const electronAPI: ElectronAPI = {
  ...
  getFileDiff: (input: import('@shadiao/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_FILE_DIFF, input)
  },

  getUntrackedContent: (input: import('@shadiao/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_UNTRACKED_CONTENT, input)
  },

  revertFile: (input: import('@shadiao/shared').RevertFileInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.REVERT_FILE, input)
  },

  getDiffContents: (input: import('@shadiao/shared').GetFileDiffInput) => {
  ...
  testMcpServer: (name: string, entry: import('@shadiao/shared').McpServerEntry) => {
  ...
  addWorktreeRepo: (workspaceSlug: string, repo: import('@shadiao/shared').WorkspaceWorktreeRepo) => {
  ...
  systemOpenFile: (filePath: string, appName?: string, access?: import('@shadiao/shared').FileAccessOptions) => {
  ...
  getDefaultAppForFile: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_DEFAULT_APP_FOR_FILE, filePath, access) as Promise<import('@shadiao/shared').DefaultAppInfo | null>
  },
  ...
  resolveAndReadFile: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-and-read', filePath, access) as Promise<{ resolvedPath: string; content: string } | null>
  },

  writeTextFile: (filePath: string, content: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:write-text', filePath, content, access) as Promise<boolean>
  },

  resolveFilePath: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-path', filePath, access) as Promise<import('@shadiao/shared').ResolvedFileUrl | null>
  },

  preparePdfPreview: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:prepare-pdf-preview', filePath, access) as Promise<{ tmpHtmlUrl: string } | null>
  },

  readBinaryBase64: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions, maxSize?: number) => {
    return ipcRenderer.invoke('file:read-binary-base64', filePath, access, maxSize) as Promise<string | null>
  },

  docxToHtml: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:docx-to-html', filePath, access) as Promise<{ resolvedPath: string; html: string } | null>
  },

  officeToHtml: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:office-to-html', filePath, access) as Promise<import('@shadiao/shared').OfficePreviewResult | null>
  },
  ...
  listAttachedDirectory: (dirPath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
  ...
  showAttachedInFolder: (filePath: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_ATTACHED_IN_FOLDER, filePath, access)
  },

  renameAttachedFile: (filePath: string, newName: string, access?: import('@shadiao/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_ATTACHED_FILE, filePath, newName, access)
  },

  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@shadiao/shared').FileAccessOptions) => {
  ...
  saveFeishuBotConfig: (input: import('@shadiao/shared').FeishuBotConfigInput) => {
  ...
  onFeishuRegisterQrcode: (callback: (payload: import('@shadiao/shared').FeishuRegisterAppQRCode) => void) => {
    const listener = (_: unknown, payload: import('@shadiao/shared').FeishuRegisterAppQRCode) => callback(payload)
  ...
  onFeishuRegisterStatus: (callback: (payload: import('@shadiao/shared').FeishuRegisterAppStatus) => void) => {
    const listener = (_: unknown, payload: import('@shadiao/shared').FeishuRegisterAppStatus) => callback(payload)
  ...
  saveDingTalkBotConfig: (input: import('@shadiao/shared').DingTalkBotConfigInput) => {
  ...
  mySkins: () => ipcRenderer.invoke('character:my-skins'),
  equipSkin: (characterId: number, skinId: number) => ipcRenderer.invoke('character:equip-skin', characterId, skinId),

  // ===== ShaDiaoAgent: 物品 =====
  getInventory: () => ipcRenderer.invoke('item:inventory'),
  equipItem: (inventoryId: number) => ipcRenderer.invoke('item:equip', inventoryId),
  unequipItem: (inventoryId: number) => ipcRenderer.invoke('item:unequip', inventoryId),
