...
if (!app.isPackaged) {
  app.setPath('userData', join(app.getPath('appData'), '@shadiao/electron-dev'))
}
...
if (!app.requestSingleInstanceLock()) {
  console.warn(
    '[启动] 已有 ShaDiaoAgent 进程持有单实例锁，本次启动将退出。\n' +
      '  如果窗口未出现，可能旧进程已卡死。请运行 `killall ShaDiaoAgent` 后重试。',
  )
  app.quit()
} else {
  // 主流程：正常启动（单实例锁已获取）
...
function registerProtocolsAndHandlers(): void {
  ...
  // 用于内联预览本地文件（renderer 用 iframe 加载 shadiao-file:// 资源）
  protocol.registerSchemesAsPrivileged([
    { scheme: 'shadiao-file', privileges: { standard: true, secure: true, supportFetchAPI: true, corsEnabled: true, stream: true } },
  ])
  ...
  app.on('second-instance', (_event, argv) => {
    // showAndFocusMainWindow removed
    const fileArg = argv.find((arg) => arg.endsWith('.shadiao-backup') || arg.endsWith('.shadiao-share'))
    if (fileArg) {
...
function handleMigrationFileOpen(filePath: string): void {
  if (filePath.endsWith('.shadiao-backup') || filePath.endsWith('.shadiao-share')) {
    sendToMainWindow(MIGRATION_IPC_OPEN, { filePath })
  }
}

// ===== Bridge 注册 =====
// IM bridges (飞书/钉钉/微信) removed in ShaDiaoAgent fork
  ...
  try {
    ...
    dialog.showErrorBox(
      ...
      `部分功能可能不可用：\n\n${message}\n\n` +
        ...
        `常见原因与排查：\n` +
        `1. 旧版 ShaDiaoAgent 进程未退出（终端运行 killall ShaDiaoAgent 后重试）\n` +
        `2. ~/.shadiao-agent/ 配置损坏（重命名 ~/.proma 后重启）\n` +
        `3. 系统 Keychain 无法解密保存的凭证（删除 ~/.shadiao-agent/feishu.json 等后重新登录）\n\n` +
...
app.on('before-quit', () => {
  ...
  // 停止所有 Bridge
  // stopBridgeSelfHealing() // bridges removed
  // stopAllBridges() // IM bridges removed
  // 停止定时任务调度器
  stopScheduler()
  // 释放飞书同步防休眠
  // stopFeishuSyncSleepBlocker() // feishu removed
  // 注销全局快捷键 removed
  // 销毁快速任务窗口 removed
