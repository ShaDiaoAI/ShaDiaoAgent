...
import { serializeCodexCredentials } from '@shadiao/shared'
import {
  listConversations,
  createConversation,
...
// import from './lib/tutorial-service' removed (ShaDiaoAgent)
import { getUserProfile, updateUserProfile } from './lib/user-profile-service'
import { getSettings, updateSettings } from './lib/settings-service'
import { setBuiltinMcpUserEnabled } from './lib/builtin-mcp/settings'
// import from './lib/dock-badge-service' removed (ShaDiaoAgent)
...
// import from './lib/github-release-service' removed (ShaDiaoAgent)
import { watchAttachedDirectory, unwatchAttachedDirectory } from './lib/workspace-watcher'
// import from './lib/feishu-config' removed (ShaDiaoAgent)
// import from './lib/feishu-bridge-manager' removed (ShaDiaoAgent)
// import from './lib/feishu-sleep-blocker' removed (ShaDiaoAgent)
// import from './lib/feishu-presence' removed
// import from './lib/dingtalk-config' removed (ShaDiaoAgent)
// import from './lib/dingtalk-bridge-manager' removed (ShaDiaoAgent)
// import from './lib/wechat-config' removed (ShaDiaoAgent)
// import from './lib/wechat-bridge' removed (ShaDiaoAgent)
 ...
 *
 * worktree 常被放在主仓库之外（如 ~/shadiao-agent-dev/worktrees/xxx），其路径不在任何
...
async function ensurePathAllowedWithWorktree(filePath: string, options?: FileAccessOptions): Promise<boolean> {
  ...
  if (mainRepo) {
    ...
    for (const workspaceSlug of getWorkspaceSlugsForAccess(options)) {
      let repos: import('@shadiao/shared').WorkspaceWorktreeRepo[]
      try {
 ...
 */
const defaultAppCache = new Map<string, import('@shadiao/shared').DefaultAppInfo>()
...
async function getMacAppIconViaSips(appPath: string): Promise<string> {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'shadiao-icon-'))
...
async function getWindowsDefaultAppInfo(filePath: string): Promise<{ appPath: string; appName: string; isUwp?: boolean } | null> {
  ...
  log.debug('ext=%s UserChoice progId=%s', ext, progId)

  if (!progId) {
    const assoc = await runCmd('cmd', ['/c', `assoc ${ext}`])
    progId = (assoc.stdout || '').split('=').slice(1).join('=').trim()
    log.debug('assoc fallback progId=%s', progId)
  }
  ...
  if (!progId) {
    ...
    if (mruOrder) {
      ...
      if (exeName && /^[a-zA-Z0-9 _.+()-]+\.exe$/i.test(exeName)) {
        ...
        log.debug('OpenWithList MRU fallback: exe=%s path=%s', exeName, exePath)
  ...
  if (!progId) {
    ...
    for (const line of owpResult.stdout.split(/\r?\n/)) {
      ...
      if (m && m[1] && isSafeWindowsProgId(m[1])) {
        progId = m[1]
        log.debug('OpenWithProgids fallback progId=%s', progId)
        break
      }
    }
  }
  if (!progId || !isSafeWindowsProgId(progId)) {
    log.debug('progId 无效或不安全:', progId)
    return null
  }
  ...
  if (!appPath) {
    ...
    log.debug('App Paths fallback: candidateAppName=%s exePath=%s', candidateAppName, exePath)
...
): Promise<import('@shadiao/shared').DefaultAppInfo | null> {
...
if let appUrl = NSWorkspace.shared.urlForApplication(toOpen: url) {
  ...
  exit(1)
    ...
    log.debug('darwin swift 结果: status=%s appPath=%s', r.status, appPath)
    ...
    log.debug('win32 getWindowsDefaultAppInfo 结果:', info)
  ...
  if (!appPath || !appName) {
    log.debug('appPath 或 appName 为空，返回 null. appPath=%s appName=%s', appPath, appName)
    return cacheNull(cacheKey)
  }

  const iconDataUrl = await getAppIconDataUrl(appPath).catch((e) => { log.warn('getAppIconDataUrl 失败:', e); return '' })
  log.debug('iconDataUrl 长度:', iconDataUrl?.length)
  if (!iconDataUrl) return cacheNull(cacheKey)

  const info: import('@shadiao/shared').DefaultAppInfo = { name: appName, appPath, iconDataUrl }
...
export function resolveAppIconPath(variantId: string): string | null {
  ...
  return join(resourcesDir, 'shadiao-logos', `proma-${variantId}.png`)
}

// ===== IM Bridge stubs (removed in ShaDiaoAgent) =====
const feishuBridgeManager = {
  ensureSessionMirror: () => Promise.resolve(),
  startSessionMirrorRun: () => Promise.resolve(),
  stopSessionMirrorRun: () => {},
  restartBot: () => Promise.resolve(),
  stopBot: () => {},
  startAll: () => Promise.resolve(),
  stopAll: () => {},
  startBot: () => Promise.resolve(),
  getStates: () => ({}),
  testConnection: () => Promise.resolve(true),
  listAllBindings: () => [],
  findBridgeByChatId: () => null,
  updateBinding: () => Promise.resolve(),
  removeBinding: () => Promise.resolve(),
} as any

const dingtalkBridgeManager = {
  restartBot: () => Promise.resolve(),
  stopBot: () => {},
  startAll: () => Promise.resolve(),
  stopAll: () => {},
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_, dirPath: string): Promise<GitRepoStatus | null> => {
      if (!dirPath || typeof dirPath !== 'string') {
        log.warn('git:get-repo-status 收到无效的目录路径')
        return null
      }
  ...
  ipcMain.handle(
    ...
    async (_, dirPath: string, sessionPath?: string, workspaceFilesPath?: string, extraPaths?: string[], sessionId?: string) => {
      if (!dirPath || typeof dirPath !== 'string') {
        log.warn('git:get-unstaged-changes 收到无效的目录路径')
  ...
  ipcMain.handle(
    ...
    async (_, input: GetFileDiffInput) => {
      ...
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string') {
        log.warn('git:get-file-diff 收到无效参数')
        return ''
      }
      const access = normalizeFileAccessOptions({ sessionId })
  ...
  ipcMain.handle(
    ...
    async (_, input: GetFileDiffInput) => {
      const { dirPath, filePath, gitRoot, sessionId } = input
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string') {
        log.warn('git:get-untracked-content 收到无效参数')
        return ''
      }
      const access = normalizeFileAccessOptions({ sessionId })
      if (!(await ensurePathAllowedWithWorktree(dirPath, access)) || (gitRoot && !(await ensurePathAllowedWithWorktree(gitRoot, access)))) return ''
  ...
  ipcMain.handle(
    ...
    async (_, input: RevertFileInput) => {
      const { dirPath, filePath, gitRoot, sessionId } = input
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string') {
        log.warn('git:revert-file 收到无效参数')
        return
      }
      const access = normalizeFileAccessOptions({ sessionId })
      if (!(await ensurePathAllowedWithWorktree(dirPath, access)) || (gitRoot && !(await ensurePathAllowedWithWorktree(gitRoot, access)))) return
  ...
  ipcMain.handle(
    ...
    async (_, input: GetFileDiffInput) => {
      const { dirPath, filePath, gitRoot, sessionId } = input
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string') {
        log.warn('git:get-diff-contents 收到无效参数')
        return null
      }
      const access = normalizeFileAccessOptions({ sessionId })
  ...
  ipcMain.handle(
    ...
    async (event, input: DetachedPreviewWindowInput): Promise<string | null> => {
      if (!input || typeof input.sessionId !== 'string' || typeof input.filePath !== 'string' || typeof input.dirPath !== 'string') {
        log.warn('preview:open-detached 收到无效参数')
        return null
      }
  ...
  ipcMain.handle(
    ...
    async (_, url: string): Promise<void> => {
      if (!url || typeof url !== 'string') {
        log.warn('shell:open-external 收到无效的 URL')
        return
      }
      // 仅允许 http/https 协议，防止安全风险
      if (!url.startsWith('http://') && !url.startsWith('https://')) {
        log.warn('shell:open-external 仅支持 http/https 协议:', url)
        return
      }
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, appName?: string, access?: FileAccessOptions | string[]): Promise<void> => {
      ...
      if (!isPathAllowed(absPath, options)) {
        log.warn('shell:system-open-file 拒绝越界路径:', absPath)
        return
      }
      if (process.platform === 'darwin') {
        ...
        if (appName) {
          if (!KNOWN_EDITORS.includes(appName)) {
            log.warn('shell:system-open-file 拒绝未知应用:', appName)
            return
          }
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@shadiao/shared').EditorApp[]> => {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@shadiao/shared').DefaultAppInfo | null> => {
      if (!filePath || typeof filePath !== 'string') return null
      try {
        const options = normalizeFileAccessOptions(access)
        if (options && !isPathAllowed(filePath, options)) {
          log.warn('shell:get-default-app-for-file 拒绝越界路径:', filePath)
          return null
        }
        log.debug('get-default-app-for-file 收到请求:', filePath)
        const result = await getDefaultAppInfoForFile(filePath, options)
        log.debug('get-default-app-for-file 返回:', result ? `name=${result.name} appPath=${result.appPath} iconLen=${result.iconDataUrl?.length}` : 'null')
        return result
      } catch (err) {
        log.warn('shell:get-default-app-for-file 失败:', err)
        return null
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, channelId: string): Promise<import('@shadiao/shared').ChannelPlanQuotaResult> => {
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@shadiao/shared').CodexOAuthLoginResult> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_, _count: number): Promise<boolean> => {
      return true // dock badge removed in ShaDiaoAgent
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, title?: string, channelId?: string, workspaceId?: string, modelId?: string): Promise<AgentSessionMeta> => {
      // 🆕 沙雕人物：如果当前选中了人物，强制使用人物 workspace。忽略渲染层传入的 workspaceId
      const effectiveWorkspaceId = selectedCharacterId != null ? `char-${selectedCharacterId}` : workspaceId
      const session = createAgentSession(title, channelId, effectiveWorkspaceId, modelId, getSettings().agentRuntime ?? 'claude', selectedCharacterId ?? undefined)
      feishuBridgeManager.ensureSessionMirror(session).catch((error) => {
        log.error('新会话建群失败:', error)
      })
      return session
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, name: string, entry: import('@shadiao/shared').McpServerEntry): Promise<{ success: boolean; message: string }> => {
  ...
  ipcMain.handle(
    ...
    async (event, input: import('@shadiao/shared').AgentQueueMessageInput): Promise<string> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: GetTaskOutputInput): Promise<GetTaskOutputResult> => {
      try {
        ...
        log.warn('GET_TASK_OUTPUT: 当前版本暂未实现，返回空输出')
        return {
          output: '',
          isComplete: false,
        }
      } catch (error) {
        log.error('获取任务输出失败:', error)
        throw error
      }
    }
  )
    ...
    async (event, response: PermissionResponse): Promise<void> => {
      ...
      if (sessionId) {
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'shadiao_event', event: { type: 'permission_resolved', requestId, behavior } },
        })
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: StopTaskInput): Promise<void> => {
      try {
        if (input.type === 'shell') {
          log.warn('STOP_TASK: Shell 任务停止功能待实现')
        } else {
          log.warn('STOP_TASK: Agent 任务暂不支持单独停止')
        }
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, mode: PromaPermissionMode): Promise<void> => {
      ...
      try {
        ...
        log.warn(`持久化 session 权限模式失败: sessionId=${sessionId}`, err)
      }
      // 若 session 正在跑，同步热切换运行时模式
      if (isAgentSessionActive(sessionId)) {
        await updateAgentPermissionMode(sessionId, mode).catch((err) => {
          log.warn(`运行中权限模式切换失败: sessionId=${sessionId}`, err)
          throw err
        })
      }
    }
  ...
  ipcMain.handle(
    ...
    async (event, response: AskUserResponse): Promise<void> => {
      ...
      if (sessionId) {
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'shadiao_event', event: { type: 'ask_user_resolved', requestId } },
        })
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (event, response: ExitPlanModeResponse): Promise<void> => {
      ...
      if (result) {
        ...
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'shadiao_event', event: { type: 'exit_plan_mode_resolved', requestId: response.requestId } },
        })
        ...
        if (targetMode) {
          ...
          if (meta) {
            try {
              ...
              log.warn(`ExitPlanMode 持久化 session 权限模式失败: sessionId=${sessionId}`, err)
            }
          }
          event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
            sessionId,
            payload: { kind: 'shadiao_event', event: { type: 'permission_mode_changed', mode: targetMode } },
          })
          log.info(`ExitPlanMode 权限模式切换: ${targetMode}`)
        }
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@shadiao/shared').PendingRequestsSnapshot> => {
      return {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, repo: import('@shadiao/shared').WorkspaceWorktreeRepo) => {
  ...
  ipcMain.handle(
    ...
    async (_, filename: string, content: string): Promise<string> => {
      ...
      const tmpDir = join(tmpdir(), 'shadiao-preview')
      ...
      log.info(`clipboard 预览文件已写入: ${tmpPath}`)
      return tmpPath
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, candidateBasePaths?: string[]): Promise<boolean> => {
      ...
      if (!existsSync(resolvedPath)) {
        log.warn('shell:show-item-in-folder 路径不存在:', resolvedPath)
        return false
      }
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, content: string, access?: FileAccessOptions | string[]): Promise<boolean> => {
      ...
      if (!resolved || !isPathAllowed(resolved, options)) {
        log.warn('file:write-text 拒绝越界路径:', resolved ?? filePath)
        return false
      }
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<ResolvedFileUrl | null> => {
      ...
      try {
        ...
        log.warn('file:resolve-path 无法注册为文件，跳过:', result, err instanceof Error ? err.message : err)
        return null
      }
    }
  )
  ...
  ipcMain.handle(
    'file:office-to-html',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@shadiao/shared').OfficePreviewResult | null> => {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, newName: string): Promise<void> => {
      ...
      log.info(`已重命名: ${safePath} → ${newPath}`)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<void> => {
      ...
      const options = normalizeFileAccessOptions(access)
      if (!isPathAllowed(safePath, options)) {
        log.warn('show-attached-in-folder 拒绝越界路径:', safePath)
        return
      }
      shell.showItemInFolder(safePath)
    }
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@shadiao/shared').FeishuBotConfigInput) => {
      ...
      if (saved.enabled && saved.appId && saved.appSecret) {
        feishuBridgeManager.restartBot(saved.id).catch((err) => {
          log.error(`Bot "${saved.name}" 重启失败:`, err)
        })
      } else {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@shadiao/shared').DingTalkBotConfigInput) => {
    ...
    try {
      ...
      if (days > 0) {
        ...
        if (archivedChats + archivedSessions > 0) {
          log.info(`已归档 ${archivedChats} 个对话, ${archivedSessions} 个 Agent 会话`)
        }
      }
    } catch (error) {
      log.error('自动归档失败:', error)
    }
  }
  ...
  ipcMain.handle('migration:cancelImport', async (_, tempDir: string) => {
    if (tempDir && existsSync(tempDir) && tempDir.includes('shadiao-import-')) {
    ...
    try {
      ...
      if (settings.autoCleanupTempOnStart !== false) {
        ...
        if (result.freedBytes > 0) {
          log.info(`启动时清理了 ${(result.freedBytes / 1024 / 1024).toFixed(1)} MB 临时文件`)
        }
      }
      ...
      if (archiveDays > 0) {
        ...
        if (result.freedBytes > 0) {
          log.info(`启动时清理了 ${(result.freedBytes / 1024 / 1024).toFixed(1)} MB 归档数据`)
        }
      }
    } catch (e) {
      log.error('启动时清理失败:', e)
  ...
  ipcMain.handle('migration:openFileDialog', async () => {
    ...
    const result = await dialog.showOpenDialog({
      title: '选择迁移文件',
      filters: [
        { name: 'ShaDiao 迁移文件', extensions: ['shadiao-backup', 'shadiao-share'] },
        { name: '所有文件', extensions: ['*'] },
      ],
      properties: ['openFile'],
    })
  ...
  ipcMain.handle('migration:saveFileDialog', async (_, mode: string) => {
    const { dialog } = await import('electron')
    const ext = mode === 'personal' ? 'shadiao-backup' : 'shadiao-share'
    const defaultName = `shadiao-migration-${new Date().toISOString().slice(0, 10)}.${ext}`
  ...
  // 否则 NaN / -Infinity / 越界值会污染 ~/.shadiao-agent/automations.json，无法回滚。
  ...
  ipcMain.handle('django:logout', async () => {
    try {
      const { logoutFromDjango } = await import('./lib/django-client.js')
      logoutFromDjango()

      // 清除主进程全局状态，避免新账号复用旧账号的人物/会话关联
      selectedCharacterId = null
      perCharacterLastSession.clear()

      // 清除持久化的每人物会话记忆和上次选中人物
      updateSettings({
      ...
      // 清除人物缓存文件，避免 API 失败时回退到旧账号的人物列表
      try {
        const { join } = await import('node:path')
        const { unlinkSync } = await import('node:fs')
        const { getConfigDir } = await import('./lib/config-paths.js')
        const charsCachePath = join(getConfigDir(), 'characters-cache.json')
        unlinkSync(charsCachePath)
      } catch { /* 文件不存在则忽略 */ }

      log.info('登出成功，auth 文件已清除，主进程状态已重置')
      return { success: true }
    } catch (e) {
      log.error('登出失败:', e)
  ...
  ipcMain.handle('character:list', async () => {
    try {
      const { listCharacters } = await import('./lib/character-service.js')
      return { success: true, data: await listCharacters() }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('character:get', async (_, id: number) => {
    try {
      const { getCharById } = await import('./lib/character-service.js')
      return { success: true, data: await getCharById(id) }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('character:create', async (_, data: any) => {
    try {
      const { createChar } = await import('./lib/character-service.js')
      return { success: true, data: await createChar(data) }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })
    ...
    try {
      ...
      if (data.name) {
        try {
          ...
          log.warn(`同步 workspace 名称失败 (char-${id}):`, e)
        }
      }
  ...
  ipcMain.handle('character:delete', async (_, id: number) => {
    try {
      const { deleteChar } = await import('./lib/character-service.js')
      await deleteChar(id)
      return { success: true }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })
  ...
  ipcMain.handle('character:skin-catalog', async () => {
    try {
      const { listSkinCatalog } = await import('./lib/character-service.js')
      return { success: true, data: await listSkinCatalog() }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('character:my-skins', async () => {
    try {
      const { listMySkins } = await import('./lib/character-service.js')
      return { success: true, data: await listMySkins() }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('character:equip-skin', async (_, characterId: number, skinId: number) => {
    try {
  ...
  ipcMain.handle('item:inventory', async () => {
    try {
      const { listInventory } = await import('./lib/character-service.js')
      return { success: true, data: await listInventory() }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('item:equip', async (_, inventoryId: number) => {
    try {
      const { equipInvItem } = await import('./lib/character-service.js')
      await equipInvItem(inventoryId)
      return { success: true }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('item:unequip', async (_, inventoryId: number) => {
    try {
      const { unequipInvItem } = await import('./lib/character-service.js')
      await unequipInvItem(inventoryId)
      return { success: true }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('reward:wallet', async () => {
    try {
      const { getWallet } = await import('./lib/character-service.js')
      return { success: true, data: await getWallet() }
    } catch (e) { return { success: false, error: (e as Error).message } }
  })

  ipcMain.handle('reward:history', async () => {
    try {
