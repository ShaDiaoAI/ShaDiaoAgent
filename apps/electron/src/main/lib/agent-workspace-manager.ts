 ...
 * - 工作区索引：~/.shadiao-agent/agent-workspaces.json（轻量元数据）
 * - 工作区目录：~/.shadiao-agent/agent-workspaces/{slug}/（Agent 的 cwd）
 */
...
/** 将 ~/.shadiao-agent/default-skills/ 的内容逐个复制到工作区 skills/ 目录 */
...
export function ensurePluginManifest(workspaceSlug: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `shadiao-workspace-${workspaceSlug}`,
    version: '1.0.0',
  }
...
interface WorkspaceConfig {
  ...
  worktreeRepos?: import('@shadiao/shared').WorkspaceWorktreeRepo[]
}
 ...
 */
export async function getWorktreeRepos(workspaceSlug: string): Promise<import('@shadiao/shared').WorkspaceWorktreeRepo[]> {
  const config = readWorkspaceConfig(workspaceSlug)

  // repoPath 归一化后去重
  const byPath = new Map<string, import('@shadiao/shared').WorkspaceWorktreeRepo>()
...
export function addWorktreeRepo(workspaceSlug: string, repo: import('@shadiao/shared').WorkspaceWorktreeRepo): import('@shadiao/shared').WorkspaceWorktreeRepo[] {
  const config = readWorkspaceConfig(workspaceSlug)
...
export function removeWorktreeRepo(workspaceSlug: string, repoPath: string): import('@shadiao/shared').WorkspaceWorktreeRepo[] {
  const config = readWorkspaceConfig(workspaceSlug)
  const existing = config.worktreeRepos ?? []
