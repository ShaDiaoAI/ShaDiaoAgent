 ...
 *
 * 负责文件附件的本地存储、读取和删除。
 * 存储路径：~/.shadiao-agent/attachments/{conversationId}/{uuid}.ext
 *
 * - 保存：base64 解码 → 写入文件
 ...
 * ~/.shadiao-agent/attachments/{conversationId}/{uuid}.ext
 *
 * @param input 保存附件参数
 ...
 *
 * 支持两种路径格式：
 * 1. 相对路径 {conversationId}/{uuid}.ext → 解析到 ~/.shadiao-agent/attachments/
 * 2. 绝对路径（Agent 工作区附件）→ 需在 ~/.shadiao-agent/ 目录下，直接读取
 *
...
export function readAttachmentAsBase64(localPath: string): string {
  ...
  if (isAbsolute(localPath)) {
    // 绝对路径：验证在 ~/.shadiao-agent/ 目录下，防止路径穿越
 ...
 * 删除对话的全部附件
 *
 * 删除整个 ~/.shadiao-agent/attachments/{conversationId}/ 目录。
 *
