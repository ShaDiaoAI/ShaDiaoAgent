 ...
 */
// Codex OAuth removed in ShaDiaoAgent fork
export async function _resolveCodexAccessToken_removed(_channelId: string): Promise<string> {
  throw new Error('Codex OAuth is not supported in ShaDiaoAgent')
}
...
async function fetchZhipuTeamQuota(
  credentials: import('@shadiao/shared').ZhipuTeamCredentials,
  proxyUrl?: string,
): Promise<ZhipuQuotaResponse | { error: string }> {
  const fetchFn = getFetchFn(proxyUrl)
  const headers: Record<string, string> = {
