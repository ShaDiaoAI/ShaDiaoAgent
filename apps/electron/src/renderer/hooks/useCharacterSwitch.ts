...
export function useCharacterSwitch(): (character: ShadiaoCharacter) => Promise<void> {
  ...
  const switchToCharacter = useCallback(async (character: ShadiaoCharacter) => {
    log.debug('===== 开始切换 =====')
    log.debug('目标:', character.name, `(id=${character.id})`)
    log.debug('currentSessionId:', currentSessionId)
    log.debug('tabsAtom:', JSON.stringify(store.get(tabsAtom).map(t => ({ id: t.id, type: t.type, sessionId: t.sessionId }))))
    log.debug('activeTabIdAtom:', store.get(activeTabIdAtom))
    ...
    log.debug('IPC 返回:', JSON.stringify(result))
    ...
    if (currentSessionId) {
      ...
      log.debug('旧会话 running?', isRunning, 'streamState:', streamState)
      ...
      if (!isRunning) {
        ...
        log.debug('找到旧 tab?', currentTab ? `id=${currentTab.id} type=${currentTab.type}` : '❌ 没找到!')
        if (currentTab) {
          ...
          log.debug('closeTab 后 tabs:', newTabs.map(t => ({ id: t.id, type: t.type })))
          log.debug('closeTab 后 activeTabId:', newActiveTabId)
          setTabs(newTabs)
          setActiveTabId(newActiveTabId)
        }
      } else {
        log.debug('旧会话 running → 保留标签页不关闭')
      }
    } else {
      log.debug('⚠️ currentSessionId 为 null/空 → 跳过关闭旧 tab')
    }

    // 3) 恢复新人物的上次会话
    if (result.lastSessionId) {
      log.debug('有历史会话:', result.lastSessionId)
      const session = sessions.find((s) => s.id === result.lastSessionId)
      if (session && !session.archived) {
        log.debug('恢复会话:', session.title)
        openSession('agent', session.id, session.title)
        return
      }
      log.debug('历史会话已归档或不存在')
    } else {
      log.debug('无历史会话')
    }

    // 4) 无历史会话：清除当前 sessionId
    log.debug('清除 currentSessionId → null')
