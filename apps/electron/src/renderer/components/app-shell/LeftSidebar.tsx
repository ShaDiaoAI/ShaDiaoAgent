...
const SESSION_QUICK_SWITCH_KEYDOWN_EVENT = 'shadiao:session-quick-switch-keydown'
const SESSION_QUICK_SWITCH_KEYUP_EVENT = 'shadiao:session-quick-switch-keyup'
...
export function LeftSidebar({ width, noTransition }: LeftSidebarProps): React.ReactElement {
  ...
  const pinnedAgentSessions = React.useMemo(
    ...
    [agentSessions, viewMode, draftSessionIds, selectedCharacter]
  )
  ...
  const agentProjectGroups = React.useMemo<AgentProjectGroup[]>(
    ...
    [agentSessions, draftSessionIds, workspaces, selectedCharacter],
  )
  ...
  if (sidebarCollapsed) {
    return (
      ...
      >
        ...
        {/* 人物切换（折叠态只显示当前人物图标，点击展开查看详情） */}
        <div className="flex flex-col items-center gap-1.5">
          <CollapsedWorkspacePopover>
            <button
              type="button"
              aria-label="当前人物（点击展开侧边栏查看）"
              onClick={() => setSidebarCollapsed(false)}
              className="relative size-10 flex items-center justify-center rounded-[12px] transition-colors titlebar-no-drag bg-primary/10 text-foreground shadow-[0_1px_2px_0_rgba(0,0,0,0.05)]"
            >
              <Bot size={18} />
            </button>
          </CollapsedWorkspacePopover>
  ...
  return (
    ...
    >
      ...
      {/* 模式切换器 + 折叠按钮 */}
	      {/* 人物信息区（替代原来的 CharacterSelector + ModeSwitcher） */}
      ...
      {characterEditTarget !== undefined && (
        <CharacterCreate
          ...
          open={characterEditTarget !== undefined}
          onClose={() => setCharacterEditTarget(undefined)}
        />
      )}
    </div>
  )
