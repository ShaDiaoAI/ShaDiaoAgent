...
import { useSmoothStream } from '@shadiao/ui'
...
import type { AgentEventUsage, RetryAttempt, SDKMessage, SDKSystemMessage } from '@shadiao/shared'
...
function AgentRunningIndicator({ startedAt }: { startedAt?: number }): React.ReactElement {
  ...
  return (
    <div className="flex items-center gap-2 min-h-[28px]">
      ...
      <span className="text-[13px] font-light text-muted-foreground/75 tabular-nums">智能体运行中 {formatTime(elapsed)}</span>
    </div>
  )
}
