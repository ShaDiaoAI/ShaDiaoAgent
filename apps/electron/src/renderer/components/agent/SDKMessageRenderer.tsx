...
// 会话转录的纯逻辑(Turn 分组 / 快照去重 / 预览)已下沉到 @shadiao/session-core 作为唯一真源。
...
import {
  ...
  type MessageGroup,
  type AssistantTurn,
} from '@shadiao/session-core'
export { groupIntoTurns, getGroupPreview, extractUserText } from '@shadiao/session-core'
export type { MessageGroup, AssistantTurn } from '@shadiao/session-core'
...
// extractMeta / MessageMeta 已迁移至 @shadiao/session-core
...
// extractUserText 已迁移至 @shadiao/session-core
...
// isUserInputMessage 已迁移至 @shadiao/session-core
...
// AssistantTurn / MessageGroup 类型已迁移至 @shadiao/session-core

// groupIntoTurns / mergeAdjacentSameModelTurns 已迁移至 @shadiao/session-core
...
const SCHEDULED_RUN_MARKER = '<!--SHADIAO_SCHEDULED_RUN-->'

// stripScheduledRunMarker 已迁移至 @shadiao/session-core（本文件从该包 import 使用）
...
export function getGroupId(group: MessageGroup): string {
  if (group.type === 'user') {
    ...
    const stableKey = (group.message as unknown as Record<string, unknown>)._shadiaoStableKey
  ...
  const stableKey = first ? (first as unknown as Record<string, unknown>)._shadiaoStableKey : undefined
  if (typeof stableKey === 'string') return stableKey
...
// getGroupPreview 已迁移至 @shadiao/session-core（本文件从该包 import 并 re-export）
