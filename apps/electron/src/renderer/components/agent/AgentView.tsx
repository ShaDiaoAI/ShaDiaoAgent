...
interface AgentThinkingPopoverProps {
  agentThinking: import('@shadiao/shared').ThinkingConfig | undefined
  onToggle: () => void
}
...
export function AgentView({ sessionId }: { sessionId: string }): React.ReactElement {
  ...
  const planQuotaChannelId = stableChannel && false
    ? stableChannel.id
    : null
  ...
  const handleCompact = React.useCallback((): void => {
    ...
    const syntheticMsg: import('@shadiao/shared').SDKMessage = {
      type: 'user',
      uuid: localUuid,
      message: {
    ...
    } as unknown as import('@shadiao/shared').SDKMessage
  ...
  const inputToolbarItems = React.useMemo<ToolbarItem[]>(() => [
    // ModelSelector 已迁至左侧面板 CharacterInfo 中，沙雕人物的模型跟着人物走
    ...(experimentalRuntimeSwitchEnabled ? [{
      key: 'runtime',
      node: (
