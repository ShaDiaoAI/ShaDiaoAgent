...
import { CodeBlock, MermaidBlock } from '@shadiao/ui'
import { detectLanguage } from '@shadiao/core'
  ...
  function MessageResponse({ children, className, basePath, basePaths, remarkPlugins }: MessageResponseProps): React.ReactElement {
    ...
    return (
      ...
      >
        ...
        >
          {normalizeLatexDelimiters(children.replace(/<!--SHADIAO_AUTOMATION:[\s\S]*?-->/g, '').trim())}
        </Markdown>
      </div>
    )
  },
