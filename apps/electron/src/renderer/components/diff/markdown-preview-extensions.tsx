...
import { highlightCode, highlightToTokens, getDisplayName } from '@shadiao/core'
import { MermaidBlock } from '@shadiao/ui'
import type { HighlightTokensResult } from '@shadiao/core'
...
function buildCodeBlockRenderModeDecorations(doc: ProseMirrorNode, editable: boolean): DecorationSet {
  ...
  doc.descendants((node, pos) => {
    ...
    decorations.push(Decoration.node(pos, pos + node.nodeSize, {
      'data-shadiao-render-mode': editable ? 'editing' : 'preview',
    }))
    return false
  })
  return DecorationSet.create(doc, decorations)
...
function isExternalUrl(src: string): boolean {
  return /^(?:https?:|data:|blob:|file:|shadiao-file:)/i.test(src)
}
...
function resolveMediaSrc(src: string, fileAccessRef: FileAccessRefOrNull, apply: (src: string) => void): () => void {
  // 外链 / data-URL / blob / 已授权 shadiao-file 协议：直接 apply，不走 IPC
...
function createShikiCodeBlockView(initialNode: ProseMirrorNode, view: EditorView) {
  ...
  dom.dataset.shadiaoCodeBlock = 'true'

  // 头部栏：语言标签 + 复制按钮
  const header = document.createElement('div')
  header.contentEditable = 'false'
  setClass(header, 'shadiao-code-header flex h-8 items-center justify-between border-b border-border/30 px-3 text-xs text-muted-foreground')
  ...
  setClass(body, 'shadiao-code-source-body markdown-code-block-body overflow-x-auto')
  ...
  setClass(mermaidHost, 'shadiao-mermaid-preview hidden')
  ...
  const render = (node: ProseMirrorNode) => {
    ...
    dom.classList.toggle('shadiao-code-block--mermaid', shouldRenderMermaid)
