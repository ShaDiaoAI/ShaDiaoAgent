...
// Proma
import ShadiaoModelLogo from '@/assets/models/proma.png'
...
import type { ProviderType } from '@shadiao/shared'
 ...
 */
export function resolveModelDisplayName(modelId: string, channels: import('@shadiao/shared').Channel[]): string {
 ...
 */
export function resolveModelProvider(modelId: string, channels: import('@shadiao/shared').Channel[]): ProviderType | undefined {
  for (const channel of channels) {
    for (const model of channel.models) {
      if (model.id === modelId) {
...
/** 默认模型图标 */
export { DefaultLogo, ShadiaoModelLogo }
